package com.example.lovecalendar.Utils.LunarUtils;
...
//        System.out.println("Date : "+Year+"/"+Month+"/"+Monthday);
        switch (Month){
            case 1:
                switch (Monthday){
                    case 1:
        ...
        int[] dateData=solarToLunar(Year,Month,Monthday);
//        System.out.println("Lunar Date : "+dateData[0]+"/"+dateData[1]+"/"+dateData[2]);
        int month=dateData[1]-1;
        int monthday=dateData[2];
        if(dateData[1]==1 && monthday==15)
            return "元宵节";
        if(dateData[1]==3 && monthday==3)
            return "上巳节";
        if(dateData[1]==5 && monthday==5)
            return "端午节";
        if(dateData[1]==7 && monthday==7)
            return "七夕节";
        if(dateData[1]==7 && monthday==15)
            return "中元节";
        if(dateData[1]==8 && monthday==15)
            return "中秋节";
        if(dateData[1]==9 && monthday==9)
            return "重阳节";
        if(dateData[1]==10 && monthday==1)
            return "寒衣节";
        if(dateData[1]==10 && monthday==15)
            return "下元节";
        if(dateData[1]==12 && monthday==8)
            return "腊八节";
        String finalDate="";
        if(monthday==1){
            if(month==0){
                ...
                finalDate=chineseNumber[month]+"月";
            }
        }else if(monthday<=10){
            finalDate=chineseTen[0]+chineseNumber[monthday-1];
        }else if(monthday<20){
            finalDate=chineseTen[1]+chineseNumber[monthday-11];
        }else if(monthday==20) {
            finalDate="二十";
        }else if(monthday<30){
            finalDate=chineseTen[2]+chineseNumber[monthday-21];
        }else{
            finalDate="三十";
        }
